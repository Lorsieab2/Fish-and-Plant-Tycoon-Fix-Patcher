#!/usr/bin/env python3
"""Manifest-driven offline patcher for the supported Windows Plant Tycoon build."""
...
EXCLUDED_OUTPUT_NAMES = {"plant tycoon - copy.exe"}
...
def resolve_paths(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    exe_name = str(target.get("exe_name", "Plant Tycoon.exe"))
    exe = game_dir / exe_name
    output_cfg = manifest.get("output", {})
    default_folder = str(output_cfg.get("default_folder_name", "Plant Tycoon - Modded"))
    output_dir = (
...
def apply_manifest(args: argparse.Namespace) -> int:
    ...
    output_exe_name = str(
        manifest.get("output", {}).get("exe_name", "Plant Tycoon - Modded.exe")
    )
    ...
    backup_dir = output_parent / ".plant_tycoon_patch_backups" / stamp
    suffix = 1
    while backup_dir.exists():
        backup_dir = output_parent / ".plant_tycoon_patch_backups" / f"{stamp}_{suffix:02d}"
        suffix += 1
    ...
    write_json(backup_dir / "plant_tycoon_patch_backup_manifest.json", backup_manifest)
    ...
    try:
        ...
        write_json(staging / "PlantTycoonFixPatchLog.json", report)
        ...
        write_json(output_dir / "PlantTycoonFixPatchLog.json", report)
    except Exception:
        if staging.exists():
...
def restore_backup(args: argparse.Namespace) -> int:
    ...
    manifest_path = backup_dir / "plant_tycoon_patch_backup_manifest.json"
    backup = read_json(manifest_path)
    source_exe_name = str(
        backup.get("source_exe_name", backup.get("exe_name", "Plant Tycoon.exe"))
    )
    output_exe_name = str(
        backup.get("output_exe_name", backup.get("exe_name", "Plant Tycoon - Modded.exe"))
    )
    ...
    write_json(output_dir / "PlantTycoonFixRestoreLog.json", report)
    print(json.dumps(report, indent=2))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Offline Plant Tycoon PC bug-fix patcher")
